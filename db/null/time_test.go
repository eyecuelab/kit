...
func TestTimeAuto(t *testing.T) {
	now := time.Now()
	valid := NewTimeAuto(now)
	if !valid.Valid {
		t.Error("unexpected valid", valid.Valid)
	}

	if valid.Time != now {
		t.Error("unexpected value", valid.Time)
	}

	invalid := NewTimeAuto(time.Time{})
	if invalid.Valid {
		t.Error("unexpected valid", invalid.Valid)
	}
}
